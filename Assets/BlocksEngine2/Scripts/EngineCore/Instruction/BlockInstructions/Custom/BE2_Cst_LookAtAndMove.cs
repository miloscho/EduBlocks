        ...
        public new void Function()
        {
            ...
            if (_counter < Mathf.Abs(Section0Inputs[1].FloatValue))
            {
                if (_timer <= 1)
                {
                    ...
                    float step = TargetObject.Transform.lossyScale.x;
                    TargetObject.Transform.position = Vector3.Lerp(_initialPosition, _initialPosition +
                                (TargetObject.Transform.forward * (Section0Inputs[1].FloatValue / Mathf.Abs(Section0Inputs[1].FloatValue)) * step), _timer);
